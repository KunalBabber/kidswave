...
import ChildFeed from "../component/ChildFeed";
...
function Home() {
  ...
  const categories = [
      "Music", "Gaming", "News", "Trending", "Entertainment", "Education",
      "Science & Tech", "Travel", "Sports", "Art"
  ];
  ...
  const handleSearchData = async (query) => {
    setLoading(true);
    try {
      // If child profile, check keywords first
      if (userData?.isChildProfile) {
        const checkResult = await axios.post(
          serverUrl + "/api/safety/search/check-keywords",
          { query },
          { withCredentials: true }
        );
        
        if (checkResult.data.isBlocked) {
          showCustomAlert("Oops! That search isn't allowed. Try something else!");
          setLoading(false);
          return;
        }
      }

      const endpoint = userData?.isChildProfile 
        ? "/api/safety/search" 
        : "/api/content/search";
      
      const payload = userData?.isChildProfile 
        ? { searchQuery: query } 
        : { input: query };
      ...
      if (error.response?.data?.isBlocked) {
        showCustomAlert("That search contains restricted content.");
      }
      setPopUp(false);
      setLoading(false);
    }
