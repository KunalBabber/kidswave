...
export const getRecommendedContent = async (req, res) => {
  try {
    ...
    // Define base query for videos and shorts based on child profile status
    let videoBaseQuery = { isApproved: true, isBlocked: false };
    let shortBaseQuery = { isApproved: true, isBlocked: false };

    if (user.isChildProfile && user.age !== null) {
      videoBaseQuery.ageLimit = { $lte: user.age };
      shortBaseQuery.ageLimit = { $lte: user.age };
    }
    ...
    const likedVideos = await Video.find({ ...videoBaseQuery, likes: userId });
    const likedShorts = await Short.find({ ...shortBaseQuery, likes: userId });
    const savedVideos = await Video.find({ ...videoBaseQuery, saveBy: userId });
    const savedShorts = await Short.find({ ...shortBaseQuery, saveBy: userId });
    ...
    if (allKeywords.length > 0) {
      allKeywords.forEach(kw => {
        videoConditions.push(
          { ...videoBaseQuery, title: { $regex: kw, $options: "i" } },
          { ...videoBaseQuery, description: { $regex: kw, $options: "i" } },
          { ...videoBaseQuery, tags: { $regex: kw, $options: "i" } }
        );
        shortConditions.push(
          { ...shortBaseQuery, title: { $regex: kw, $options: "i" } },
          { ...shortBaseQuery, tags: { $regex: kw, $options: "i" } }
        );
      });
    }
